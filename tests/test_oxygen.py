...
def test_migrate_processes_remaining_files_after_hash_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    (tmp_path / "good.jpg").write_bytes(b"good")
    (tmp_path / "bad.jpg").write_bytes(b"bad")
    original_digest = __import__(
        "photostow.oxygen", fromlist=["_stable_digest"]
    )._stable_digest

    def selective_digest(path: Path):
        if path.name == "bad.jpg":
            raise OSError("bad input")
        return original_digest(path)

    monkeypatch.setattr("photostow.oxygen._stable_digest", selective_digest)
    with pytest.raises(OSError, match="migration failed"):
        migrate(tmp_path, verbose=True)
...
def test_migrate_manifest_missing_parent_fails_cleanly(tmp_path: Path) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    parent = tmp_path / "missing-parent"
    manifest = parent / "review.json"

    with pytest.raises(FileNotFoundError):
        migrate(tmp_path, manifest=manifest)
    assert not parent.exists()


def test_migrate_manifest_temp_is_private(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    ...
    def record_mode(source: Path, destination: Path) -> None:
    ...
    monkeypatch.setattr("photostow.oxygen.os.replace", record_mode)
    monkeypatch.setattr("photostow.oxygen.os.fsync", record_fsync)
...
def test_migrate_manifest_replace_failure_cleans_temp(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
...
def test_migrate_manifest_directory_fsync_failure_cleans_temp(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    ...
    monkeypatch.setattr("photostow.oxygen.os.fsync", fail_directory_fsync)
...
def test_migrate_writes_review_manifest(tmp_path: Path) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    manifest = tmp_path / "trial.json"

    assert migrate(tmp_path, manifest=manifest) == 1
    record = json.loads(manifest.read_text(encoding="utf-8"))["records"][0]
    assert record["path"] == "photo.jpg"
...
def test_migrate_cli_reports_relative_manifest_root(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    manifest.write_text(
        json.dumps({"root": str(tmp_path / "archive")}), encoding="utf-8"
    )
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(
        "sys.argv", ["oxygen-migrate", "--apply", "manifest.json", "--nice", "0"]
    )
    monkeypatch.setattr("photostow.oxygen_cli.migrate", lambda *args, **kwargs: 0)
...
def test_migrate_cli_reports_malformed_manifest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    manifest = tmp_path / "broken.json"
    manifest.write_text("{broken", encoding="utf-8")
    monkeypatch.setattr("sys.argv", ["oxygen-migrate", "--apply", str(manifest)])

    assert migrate_main() == 1
    captured = capsys.readouterr()
    assert captured.out == ""
    assert "oxygen-migrate failed:" in captured.err
...
def test_migrate_cli_handles_object_creation_race(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["oxygen-migrate", "2006", "--root", str(tmp_path), "--apply", "--nice", "0"],
    )

    assert migrate_main() == 0
    assert photo.stat().st_nlink == 2
...
def test_migrate_cli_requires_target_without_manifest(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("sys.argv", ["oxygen-migrate"])
    with pytest.raises(SystemExit) as error:
        migrate_main()
    assert error.value.code == 2


def test_migrate_cli_applies_manifest_twice(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "archive"
    source = root / "2006" / "photo.jpg"
    source.parent.mkdir(parents=True)
    source.write_bytes(b"photo")
    manifest = tmp_path / "review.json"
    assert migrate(root, dry_run=True, manifest=manifest) == 1
    monkeypatch.setattr(
        "sys.argv", ["oxygen-migrate", "--apply", str(manifest), "--nice", "0"]
    )
...
def test_migrate_cli_rejects_missing_manifest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_migrate_cli_rejects_manifest_root_mismatch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
        ...
        json.dumps(
            {
                ...
                "object_root": str(manifest_root / ".objects"),
                "records": [],
            }
        ),
        encoding="utf-8",
    ...
    assert migrate_main() == 1
    assert "manifest root does not match root" in capsys.readouterr().err


def test_migrate_cli_applies_object_root_override(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "archive"
    ...
    root.mkdir()
    source.write_bytes(b"photo")
    manifest_objects = tmp_path / "manifest-objects"
    manifest_objects.mkdir()
    override_objects = tmp_path / "override-objects"
    override_objects.mkdir()
    manifest = tmp_path / "review.json"
    assert migrate(root, manifest=manifest, object_root=manifest_objects) == 1
    monkeypatch.setattr(
        "sys.argv",
        [
            "oxygen-migrate",
    ...
    assert migrate_main() == 0
    digest = hashlib.sha256(b"photo").hexdigest()
    assert object_path(root, digest, override_objects).is_file()
    assert not object_path(root, digest, manifest_objects).exists()


def test_migrate_cli_rejects_manifest_record_symlink(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    ...
    assert migrate_main() == 1
    assert "manifest path is outside root" in capsys.readouterr().err


def test_migrate_cli_rejects_missing_manifest_record_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    root.mkdir()
...
def test_migrate_cli_rejects_manifest_file_root(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "not-a-directory"
    root.write_bytes(b"file root")
    manifest = tmp_path / "review.json"
    manifest.write_text(
        json.dumps(
            {
...
def test_migrate_cli_rejects_manifest_file_object_root(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    ...
    assert migrate_main() == 1
    assert str(object_root) in capsys.readouterr().err


def test_migrate_cli_rejects_manifest_filesystem_mismatch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    ...
    monkeypatch.setattr(
        "photostow.oxygen._check_same_filesystem",
        lambda *args: (_ for _ in ()).throw(OSError("different filesystem")),
    )
    monkeypatch.setattr(
        "sys.argv", ["oxygen-migrate", "--apply", str(manifest), "--nice", "0"]
    )
    ...
    assert migrate_main() == 1
    assert "different filesystem" in capsys.readouterr().err


def test_migrate_cli_rejects_manifest_symlink_object_root(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    root.mkdir()
...
def test_migrate_cli_rejects_manifest_path_outside_root(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    root.mkdir()
...
def test_migrate_cli_rejects_unknown_manifest_schema(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_cli_rejects_incomplete_manifest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_cli_accepts_same_digest_manifest_mtime_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    source = root / "photo.jpg"
    ...
    source.write_bytes(b"photo")
    manifest = tmp_path / "review.json"
    assert migrate(root, manifest=manifest) == 1
    stat = source.stat()
...
def test_migrate_cli_accepts_same_content_inode_replacement(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "archive"
    source = root / "photo.jpg"
    ...
    replacement.write_bytes(b"photo")
...
def test_migrate_cli_applies_external_object_root_manifest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "archive"
    source = root / "photo.jpg"
    root.mkdir()
    source.write_bytes(b"photo")
    object_root = tmp_path / "external-objects"
    object_root.mkdir()
    manifest = tmp_path / "review.json"
    assert migrate(root, manifest=manifest, object_root=object_root) == 1
    monkeypatch.setattr(
        "sys.argv", ["oxygen-migrate", "--apply", str(manifest), "--nice", "0"]
    )
    ...
    assert source.stat().st_nlink == 2
    digest = hashlib.sha256(b"photo").hexdigest()
    assert object_path(root, digest, object_root).is_file()


def test_migrate_cli_rejects_changed_manifest_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "archive"
    source = root / "photo.jpg"
    ...
    assert "file changed since manifest" in capsys.readouterr().err


def test_migrate_cli_rejects_apply_and_dry_run(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        "sys.argv", ["oxygen-migrate", str(tmp_path), "--apply", "--dry-run"]
    )
    with pytest.raises(SystemExit) as error:
        migrate_main()
    assert error.value.code == 2
...
def test_migrate_rejects_duplicate_manifest_paths(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    record = {
    ...
    with pytest.raises(ValueError) as error:
        migrate(root, dry_run=False, apply_manifest=manifest)
    assert str(error.value) == "manifest contains duplicate paths"


def test_migrate_rejects_boolean_manifest_metadata(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
    ...
    with pytest.raises(ValueError) as error:
        migrate(root, dry_run=False, apply_manifest=manifest)
    assert str(error.value) == "unsupported manifest schema"


def test_migrate_rejects_negative_manifest_metadata(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_empty_manifest_record_path(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_absolute_manifest_record_path(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_invalid_manifest_digest(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_non_string_manifest_record_path(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_non_string_manifest_root(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
        json.dumps({"version": 1, "root": [], "object_root": "objects", "records": []}),
        encoding="utf-8",
    )
...
def test_migrate_rejects_relative_manifest_root(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_relative_manifest_object_root(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_empty_manifest_object_root(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_boolean_manifest_version(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    manifest.write_text(
...
def test_migrate_rejects_non_object_apply_manifest(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
...
def test_migrate_rejects_invalid_utf8_apply_manifest(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    ...
    with pytest.raises(ValueError) as error:
        migrate(root, dry_run=False, apply_manifest=manifest)
    assert str(error.value) == "apply manifest is not valid UTF-8"


def test_migrate_rejects_invalid_json_apply_manifest(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    manifest = tmp_path / "review.json"
    ...
    with pytest.raises(ValueError) as error:
        migrate(root, dry_run=False, apply_manifest=manifest)
    assert str(error.value) == "apply manifest is not valid JSON"


def test_migrate_rejects_symlink_apply_manifest_parent(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    target = tmp_path / "manifests"
    target.mkdir()
    manifest_target = target / "review.json"
    manifest_target.write_text("{}", encoding="utf-8")
    parent = tmp_path / "linked"
    parent.symlink_to(target, target_is_directory=True)

    with pytest.raises(ValueError) as error:
        migrate(root, dry_run=False, apply_manifest=parent / "review.json")
    assert str(error.value) == (
        f"refusing symlink apply manifest parent: {parent / 'review.json'}"
    )


def test_migrate_rejects_symlink_apply_manifest(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    ...
    with pytest.raises(ValueError) as error:
        migrate(root, dry_run=False, apply_manifest=manifest)
    assert str(error.value) == f"refusing symlink apply manifest: {manifest}"


def test_migrate_apply_manifest_is_safe_to_rerun(tmp_path: Path) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    manifest = tmp_path / "trial.json"
    migrate(tmp_path, manifest=manifest)

    assert migrate(tmp_path, dry_run=False, apply_manifest=manifest) == 1
    assert migrate(tmp_path, dry_run=False, apply_manifest=manifest) == 1


def test_migrate_apply_manifest_rejects_symlink_parent(tmp_path: Path) -> None:
    root = tmp_path / "archive"
    root.mkdir()
    target = root / "target"
    target.mkdir()
    ...
    photo.write_bytes(b"photo")
    manifest = tmp_path / "trial.json"
    migrate(root, manifest=manifest)
    link = root / "linked"
    link.symlink_to(target, target_is_directory=True)
    manifest_data = json.loads(manifest.read_text(encoding="utf-8"))
    manifest_data["records"][0]["path"] = "linked/photo.jpg"
    manifest.write_text(json.dumps(manifest_data), encoding="utf-8")

    with pytest.raises(OSError, match="manifest path has symlinked parent"):
        migrate(root, dry_run=False, apply_manifest=manifest)


def test_migrate_apply_manifest_rejects_symlink_path(tmp_path: Path) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    manifest = tmp_path / "trial.json"
    migrate(tmp_path, manifest=manifest)
    ...
    external.write_bytes(b"photo")
    photo.unlink()
    photo.symlink_to(external)

    with pytest.raises(OSError, match="manifest path is a symlink"):
        migrate(tmp_path, dry_run=False, apply_manifest=manifest)


def test_migrate_rejects_incomplete_manifest_record(tmp_path: Path) -> None:
    manifest = tmp_path / "trial.json"
    manifest.write_text(
        json.dumps(
            {
...
def test_migrate_rejects_unknown_manifest_schema(tmp_path: Path) -> None:
    manifest = tmp_path / "trial.json"
    manifest.write_text(
        json.dumps({"version": 2, "root": str(tmp_path), "records": []}),
        encoding="utf-8",
    )
...
def test_migrate_applies_manifest_and_rejects_changes(tmp_path: Path) -> None:
    photo = tmp_path / "photo.jpg"
    photo.write_bytes(b"photo")
    manifest = tmp_path / "trial.json"
    migrate(tmp_path, manifest=manifest)
    ...
    with pytest.raises(OSError, match="changed since manifest"):
        migrate(tmp_path, dry_run=False, apply_manifest=manifest)
